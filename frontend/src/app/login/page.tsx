  ...
  const handleGoogleSuccess = async (tokenResponse: any) => {
    try {
      ...
      if (res.ok) {
        ...
        toast.success("Login berhasil! Selamat datang di Raja Brukat.");
        router.push("/profile");
      } else {
        const err = await res.json();
        setError(err.error || "Google login gagal");
        toast.error(err.error || "Google login gagal");
      }
    } catch (err) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        const errorMsg = err.error || "Email atau kata sandi tidak valid";
        setError(errorMsg);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.user.role === "ADMIN") {
          ...
          toast.success("Selamat datang kembali, Admin Raja Brukat!");
        ...
        >
          <div className="mb-8 text-center lg:text-left">
            ...
            <p className="text-stone-500 text-sm font-normal">
              Masukkan email dan kata sandi Anda untuk mengakses akun Raja Brukat.
            </p>
          </div>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex justify-end pt-1">
                    <a
                      href="#"
                      className="text-xs font-semibold text-[#b77305] hover:underline transition-colors"
                    >
                      Lupa kata sandi?
                    </a>
                  </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-xs font-medium text-stone-500">
              Belum memiliki akun Raja Brukat?{" "}
              <Link
                href="/register"
