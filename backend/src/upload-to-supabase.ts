...
if (!SUPABASE_KEY) {
  console.error("❌ ERROR: SUPABASE_SERVICE_ROLE_KEY (atau SUPABASE_ANON_KEY) di file backend/.env masih kosong!");
  console.error("Silakan isi SUPABASE_SERVICE_ROLE_KEY di file backend/.env terlebih dahulu.");
  process.exit(1);
}
...
async function uploadLocalFileToSupabase(localFilePath: string, destFileName: string): Promise<string | null> {
  try {
    ...
    // Convert local PNG/JPG file to WebP format in-memory with high compression
...
async function main() {
  console.log(`☁️ Connected to Supabase Storage Bucket: "${BUCKET_NAME}" at ${SUPABASE_URL}`);

  try {
    const { data: buckets } = await supabase.storage.listBuckets();
    const exists = buckets?.some(b => b.name === BUCKET_NAME);
    if (!exists) {
      console.log(`Creating public bucket "${BUCKET_NAME}" in Supabase Storage...`);
      await supabase.storage.createBucket(BUCKET_NAME, { public: true });
    }
  } catch (bErr: any) {
    console.log(`Bucket check info:`, bErr?.message || bErr);
  }
  ...
  console.log("Cleaning existing database products...");
    ...
    try {
      ...
      if (Array.isArray(data.variants) && data.variants.length > 0) {
        for (let idx = 0; idx < data.variants.length; idx++) {
          ...
          if (imageSourcePath && fs.existsSync(imageSourcePath)) {
            ...
            const publicUrl = await uploadLocalFileToSupabase(imageSourcePath, destFilename);
            if (publicUrl) {
      ...
      if (galleryImages.length === 0) {
        ...
        for (let idx = 0; idx < localImagesInDir.length; idx++) {
          ...
          const publicUrl = await uploadLocalFileToSupabase(srcPath, destFilename);
          if (publicUrl) {
            galleryImages.push(publicUrl);
            totalUploadedImages++;
          }
      ...
      successCount++;
      console.log(`[${successCount}/${metadataPaths.length}] Uploaded ${galleryImages.length} images to Supabase: ${title.substring(0, 45)}...`);
    } catch (err: any) {
      console.error(`Failed to upload/import ${metadataPath}:`, err?.message || err);
    }
  }

  console.log(`\n🎉 SUCCESS! Successfully uploaded ${totalUploadedImages} local images to Supabase Storage and imported ${successCount} products into database!`);
}
