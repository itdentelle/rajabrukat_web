...
function cleanTitle(titleStr: string): string {
  ...
  // If title was only a URL or junk string, fallback to clean name
  if (!cleaned || cleaned.length < 4) {
    return "";
  }

  // Remove long trailing buzzwords
...
async function main() {
  ...
  for (let i = 0; i < metadataPaths.length; i++) {
    ...
    try {
      ...
      if (!formattedTitle) {
        console.log(`⏭️ Skipping invalid title product in: "${folderName}"`);
        skippedJunkCount++;
        continue;
      }
      ...
      // Skip products that have no valid images at all
      if (galleryImages.length === 0) {
        console.log(`⏭️ Skipping product without valid local images: "${formattedTitle}"`);
        skippedJunkCount++;
        continue;
      }
      ...
      successCount++;
      console.log(`[${successCount}] Imported: "${formattedTitle.substring(0, 45)}" -> Category: [${category}] (${galleryImages.length} images)`);
    } catch (err: any) {
      console.error(`Failed for ${metadataPath}:`, err?.message || err);
    }
  }

  console.log(`\n🎉 DONE! Cleaned & Imported ${successCount} authentic products to Supabase DB (Skipped ${skippedJunkCount} junk/broken folders).`);
}
...
main()
  ...
  .catch(async (e) => {
    console.error("Clean import error:", e);
    await prisma.$disconnect();
    process.exit(1);
  });
